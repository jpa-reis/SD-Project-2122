...
import tp1.server.interoperable.clients.files_clients.FilesClientFactory;
import tp1.server.interoperable.clients.users_clients.UsersClientFactory;
...
public class JavaDirectories implements Directory{
    ...
    public Result<byte[]> getFile(String filename, String userId, String accUserId, String password) {
        ...
        FilesClientFactory filesClientFactory = new FilesClientFactory();
    ...
    private Result<User> clt_checkUser(String userId, String password){
        ...
        UsersClientFactory usersClient = new UsersClientFactory();
        Result<User> r = usersClient.getClient(userServiceURIS[0]).getUser(userId, password);
        return r;
    }
    ...
    private FileInfo clt_writeFile(String userId, String filename, byte[] data, boolean failed){
        ...
        Log.info(fileServiceURIS[0].toString());
        ...
        FilesClientFactory filesClient = new FilesClientFactory();
        filesClient.getClient(URI.create(minServer), fileId, canRedirect).writeFile(fileId, data, "");
    ...
    private void clt_deleteFile(String userId, String filename){
        FilesClientFactory filesClient = new FilesClientFactory();
        filesClient.getClient(URI.create(filesInfo.get(userId).get(filename).getFileURL()), String.format("%s_%s", userId, filename), canRedirect).deleteFile(String.format("%s_%s", userId, filename), "");
    }
